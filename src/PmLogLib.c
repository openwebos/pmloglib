...
static PmLogContext libProcessContext = kPmLogGlobalContext;
...
void CallSysLog(const char *context, const int level, const char* pidtid, const char* fmt, ...)
{
	...
	int index = 0;
	va_list args;
	index = snprintf(buffer, sizeof(buffer), "%s %s %s ", pidtid, PMLOG_IDENTIFIER, context);
	va_start (args, fmt);
...
void GetPidStr(PmLogContext_ *context,char *ptidStr,long int ptidStrLen)
{
	...
	if ((context->info.flags & kPmLogFlag_LogProcessIds) ||
		...
		if (context->info.flags & kPmLogFlag_LogThreadIds &&
		    (tid != pid)) {
			mysprintf(ptidStr, ptidStrLen, "[%d:%d]", (int) pid,
			    (int) tid);
		} else {
			mysprintf(ptidStr, ptidStrLen, "[%d]", (int) pid);
		}
	} else {
		g_strlcpy(ptidStr, "[]", ptidStrLen);
	}
}
...
static void mysprintf(char* dst, size_t dstSize, const char* fmt, ...)
{
	...
	if (dst == NULL)
	{
		DbgPrint("mysprintf null dst\n");
		return;
	}

	if (dstSize < 1)
	{
		DbgPrint("mysprintf invalid dst size\n");
		return;
	}
	...
	if (fmt == NULL)
	{
		DbgPrint("mysprintf null fmt\n");
		return;
	}

	va_start(args, fmt);
	g_vsnprintf(dst, dstSize, fmt, args);
	va_end(args);
}
...
{
	gchar *is_nfs = NULL;
	bool is_squashfs = false;
	bool is_official = false;
	errno = 0;

	gchar **env = g_get_environ();

	is_nfs = g_environ_getenv(env, "IS_NFS");

	// is flash?
	if (is_nfs)
	{
		is_squashfs = (strncmp(is_nfs, "NO", strlen(is_nfs)) == 0);
	}
	...
	nyx_error_t nyx_error = NYX_ERROR_GENERIC;
	const char *build_id = NULL;
	int build_number = 0;

	nyx_init();

	nyx_error = nyx_device_open(NYX_DEVICE_OS_INFO, "Main", &device);

	if (nyx_error != NYX_ERROR_NONE)
	{
		ErrPrint(COMPONENT_PREFIX, "[]", "NYX_DEVICE_OPEN_ERROR {\"ErrorCode\":\"%d\"}", nyx_error);
	}
	else
	{
		nyx_error = nyx_os_info_query(device, NYX_OS_INFO_WEBOS_BUILD_ID, &build_id);
		if (nyx_error != NYX_ERROR_NONE)
		{
			ErrPrint(COMPONENT_PREFIX, "[]", "NYX_GET_BUILDID_ERROR {\"ErrorCode\":\"%d\"}", nyx_error);
		}
	}

	nyx_deinit();

	build_number = g_ascii_strtoull(build_id, NULL, 10);
	is_official = (build_number > 0);

	// is the flag file exists? && is flash? & is official?
	return (is_squashfs && is_official);
}
#endif
...
#ifndef WEBOS_MASS_PRODUCTION // Parse only default.conf in production
...
static void __attribute ((constructor)) init_function(void)
{
	...
	if (gGlobalsP->signature == 0)
	{
		...
		gGlobalsP->contextLogging = false;
/*
...
#pragma GCC diagnostic pop
		...
		gGlobalContextP->info.enabledLevel = kPmLogLevel_Info;
	...
	*pContext = NULL;
	...
	if (theContextP != NULL)
	{
		*pContext = PrvExportContext(theContextP);
		...
		if (libProcessContext == kPmLogGlobalContext)
		{                                                             //if libProcessContext is not set using PmlogSetLibContext
	                libProcessContext = *pContext;
		}
		return kPmLogErr_None;
	}
	...
	if (level != kPmLogLevel_Debug) {
		...
		if (kvpairs) {
			if (!validate_json_string(kvpairs, &logErr, false)) {
				char truncated[TRUNCATED_MSG_SIZE] = {0,};
				gchar *escaped_str = NULL;
				char *err_str = NULL;

				snprintf(truncated, sizeof(truncated), "%s", kvpairs);
				escaped_str = g_strescape(truncated, NULL);
...
static bool validate_format(size_t kv_count, const char *format)
{
#ifndef WEBOS_MASS_PRODUCTION
	size_t count = 0;
                     ...
                     const char *check_formats, const char *fmt, ...)
{
	...
	if (kPmLogLevel_Debug != level) {
		...
		if (kv_count) {
			...
			if (!validate_format(kv_count, check_formats)) {
				ErrPrint(context_ptr->component, ptidStr, "MISMATCHED_FMT {\"MSGID\":\"%s\"}",
				         msgid);
				return kPmLogErr_InvalidFormat;
			}
	...
	if (ret < 0) {
		...
		gchar *escaped_str = NULL;
		snprintf(buffer, sizeof(buffer), "%s", ptr_final_str);
		escaped_str = g_strescape(buffer, NULL);
		WarnPrint(context_ptr->component, ptidStr, "MSG_TRUNCATED {\"MSGID\":\"%s\",\"CAUSE\":\"Log message exceeded 1024 bytes\",\"TRUNCATED_MSG\":\"%s ...\"}",
	...
	if (kPmLogLevel_Debug != level) {
		...
		if (kv_count != 0) {
			if (!validate_json_string(final_str, &err, true)) {
				...
				snprintf(truncated, sizeof(truncated), "%s", final_str);
				escaped_str = g_strescape(truncated, NULL);
